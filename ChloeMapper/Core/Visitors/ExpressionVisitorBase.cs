    ...
    public class ExpressionVisitorBase : ExpressionVisitor<DbExpression>
    {
        ...
        protected override DbExpression VisitBinary_And(BinaryExpression exp)
        {
            if (exp.Type == PublicConstants.TypeOfBoolean)
                return this.Visit(System.Linq.Expressions.Expression.AndAlso(exp.Left, exp.Right, exp.Method));
            else
        ...
        protected override DbExpression VisitBinary_AndAlso(BinaryExpression exp)
        {
            ...
            System.Linq.Expressions.Expression left = exp.Left, right = exp.Right;
            ...
            if (c != null)
            {
                if ((bool)c.Value == true)
                {
                    ...
                    dbExp = this.Visit(System.Linq.Expressions.Expression.Equal(exp.Left, UtilConstants.Constant_True));
                    return dbExp;
                }
                else
                {
            ...
            if (c != null)
            {
                ...
                {
                    // (a.ID==1)==true
                    dbExp = this.Visit(System.Linq.Expressions.Expression.Equal(exp.Right, UtilConstants.Constant_True));
                    return dbExp;
                }
                else
                {
            ...
            // left==true
            var newLeft = System.Linq.Expressions.Expression.Equal(left, UtilConstants.Constant_True);
            // right==true
            var newRight = System.Linq.Expressions.Expression.Equal(right, UtilConstants.Constant_True);
        ...
        {
            if (exp.Type == PublicConstants.TypeOfBoolean)
                return this.Visit(System.Linq.Expressions.Expression.OrElse(exp.Left, exp.Right, exp.Method));
            else
        ...
        protected override DbExpression VisitConditional(ConditionalExpression exp)
        {
            ...
            System.Linq.Expressions.Expression test = exp.Test, ifTrue = exp.IfTrue, ifFalse = exp.IfFalse;
            System.Linq.Expressions.Expression whenTrueTest = null, whenFalseTest = null, thenIfTrue = null, thenfFalse = null;

            // test==true
            whenTrueTest = System.Linq.Expressions.Expression.Equal(test, UtilConstants.Constant_True);
            whenFalseTest = System.Linq.Expressions.Expression.Equal(test, UtilConstants.Constant_False);
        ...
        protected override DbExpression VisitMethodCall(MethodCallExpression exp)
        {
            if (exp.Method == PublicConstants.MethodInfo_String_IsNullOrEmpty)
            {
                ...
                var equalNullExp = System.Linq.Expressions.Expression.Equal(stringArg, UtilConstants.Constant_Null_String);
                var equalEmptyExp = System.Linq.Expressions.Expression.Equal(stringArg, UtilConstants.Constant_Empty_String);
                return this.Visit(System.Linq.Expressions.Expression.OrElse(equalNullExp, equalEmptyExp));
            }
        ...
        DbExpression VisitBinary_Specific(System.Linq.Expressions.Expression exp, bool trueOrFalse)
        {
            // a.B == true      a.B=1
            // !a.B==true        not (a.b=1)
            ...
            else
            {
                ...
                if (exp.NodeType == ExpressionType.MemberAccess)
                {
                    ...
                    {
                        var nullRight = System.Linq.Expressions.Expression.Constant(null, memberExpression.Expression.Type);
                        if (trueOrFalse)
                            return this.Visit(System.Linq.Expressions.Expression.NotEqual(memberExpression.Expression, nullRight));
                        else
                            return this.Visit(System.Linq.Expressions.Expression.Equal(memberExpression.Expression, nullRight));
                    }
                    else
                    {
        ...
        // isNullable 
        DbExpression BuildExpForWhenBooleanEqual(System.Linq.Expressions.Expression left, System.Linq.Expressions.Expression right, bool isNullable)
        {
            System.Linq.Expressions.Expression resultExp = null;

            // left==true && right==true
            var nLeft_Or = System.Linq.Expressions.Expression.AndAlso(BuildBoolEqual(left, true), BuildBoolEqual(right, true));

            // left==false && right==false
            var nRigth_Or = System.Linq.Expressions.Expression.AndAlso(BuildBoolEqual(left, false), BuildBoolEqual(right, false));

            // left==true && right==true) || (left==false && right==false
            resultExp = System.Linq.Expressions.Expression.OrElse(nLeft_Or, nRigth_Or);
            ...
            if (isNullable && (ExpressionExtension.StripConvert(left)).NodeType == ExpressionType.MemberAccess && (ExpressionExtension.StripConvert(right)).NodeType == ExpressionType.MemberAccess)
            {
                ...
                resultExp = System.Linq.Expressions.Expression.OrElse(resultExp, System.Linq.Expressions.Expression.AndAlso(System.Linq.Expressions.Expression.Equal(left, UtilConstants.Constant_Null_Boolean), System.Linq.Expressions.Expression.Equal(right, UtilConstants.Constant_Null_Boolean)));
            }
        ...
        // left 和 right 的 Type 必须为 bool 或者 Nullable<bool>
        // isNullable 
        DbExpression BuildExpForWhenBooleanNotEqual(System.Linq.Expressions.Expression left, System.Linq.Expressions.Expression right, bool isNullable)
        {
            System.Linq.Expressions.Expression resultExp = null;

            // left==true && right==false
            var nLeft_Or = System.Linq.Expressions.Expression.AndAlso(BuildBoolEqual(left, true), BuildBoolEqual(right, false));

            // left==false && right==true
            var nRigth_Or = System.Linq.Expressions.Expression.AndAlso(BuildBoolEqual(left, false), BuildBoolEqual(right, true));
            ...
            if (isNullable && (ExpressionExtension.StripConvert(left)).NodeType == ExpressionType.MemberAccess && (ExpressionExtension.StripConvert(right)).NodeType == ExpressionType.MemberAccess)
            {
                ...
                resultExp = System.Linq.Expressions.Expression.OrElse(resultExp, System.Linq.Expressions.Expression.AndAlso(System.Linq.Expressions.Expression.Equal(left, UtilConstants.Constant_Null_Boolean), System.Linq.Expressions.Expression.NotEqual(right, UtilConstants.Constant_Null_Boolean)));
                resultExp = System.Linq.Expressions.Expression.OrElse(resultExp, System.Linq.Expressions.Expression.AndAlso(System.Linq.Expressions.Expression.NotEqual(left, UtilConstants.Constant_Null_Boolean), System.Linq.Expressions.Expression.Equal(right, UtilConstants.Constant_Null_Boolean)));
            }
        ...
        static System.Linq.Expressions.Expression BuildBoolEqual(System.Linq.Expressions.Expression left, bool trueOrFalse)
        {
            if (left.Type == PublicConstants.TypeOfBoolean)
            {
                if (trueOrFalse)
                    return System.Linq.Expressions.Expression.Equal(left, UtilConstants.Constant_True);
                else
                    return System.Linq.Expressions.Expression.Equal(left, UtilConstants.Constant_False);
            }

            if (left.Type == PublicConstants.TypeOfBoolean_Nullable)
            {
                if (trueOrFalse)
                    return System.Linq.Expressions.Expression.Equal(left, UtilConstants.Convert_TrueToNullable);
                else
                    return System.Linq.Expressions.Expression.Equal(left, UtilConstants.Convert_FalseToNullable);
            }
