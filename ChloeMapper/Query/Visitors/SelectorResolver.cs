...
namespace Dapper.Extensions.Expression.Query.Visitors
{
    class SelectorResolver : ExpressionVisitor<IObjectModel>
    {
        ...
        DbExpression ResolveExpression(System.Linq.Expressions.Expression exp)
        {
        ...
        public override IObjectModel Visit(System.Linq.Expressions.Expression exp)
        {
            if (exp == null)
        ...
        protected override IObjectModel VisitNew(NewExpression exp)
        {
            ...
            for (int i = 0; i < parames.Length; i++)
            {
                ...
                System.Linq.Expressions.Expression argExp = exp.Arguments[i];
        ...
        IObjectModel VisistMapTypeSelector(System.Linq.Expressions.Expression exp)
        {
            if (!MappingTypeSystem.IsMappingType(exp.Type))
            {
