...
{
    [TestClass]
    public class Join2QueryTests : MsSqlBaseTest
    {
        /// <summary>
        /// 查询测试
        /// </summary>
        ...
        public void JoinQueryNewArrayContainsTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            query.Where((v, c) => new[] { "CDA589B9F0106E7A", "FD094A06D124C5" }.Contains(v.SerialNo));
        ...
        public void JoinQueryNewListContainsTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            query.Where((a, b) => new List<string> { "CDA589B9F0106E7A", "FD094A06D124C5" }.Contains(a.SerialNo));
            IEnumerable<Order> data = query.Select((a, b) => a).ToList<Order>();
            Assert.IsTrue(data.Any());
        }
        ...
        public void JoinQueryIEnumerableContainsTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IEnumerable<string> values = new[] { "67", "24" };
        ...
        public async Task JoinQueryPageTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            query.OrderByDescending((a, b) => a.CreateTime).TakePage(1, 10);
        ...
        public async Task JoinQueryTakeTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IEnumerable<Order> result = await query.Select((a, b) => a).OrderByDescending((a, b) => a.CreateTime).Take(100).ToListAsync<Order>();
        ...
        public async Task JoinQueryGroupTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            query.OrderByDescending((a, b) => a.CreateTime).TakePage(1, 10).Select((a, b) => new { a.CreateTime }).GroupBy((a, b) => a.CreateTime);
            IList<Order> result = await query.ToListAsync<Order>();
            Assert.IsTrue(result.Any());
        }
        ...
        public async Task JoinQueryGroupHavingTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IEnumerable<Order> result = await query.Select((a, b) => new { a.CreateTime, Total = Function.Count() }).GroupBy((f, g) => f.CreateTime).Having((f, d) => f.CreateTime > new DateTime(2021, 3, 10)).ToListAsync<Order>();
            Assert.IsTrue(result.Any());
        }
        ...
        public async Task JoinQueryDateTimeDateTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IEnumerable<Order> result = await query.Select((a, b) => new { a.CreateTime }).Where((f, d) => f.CreateTime.Date > new DateTime(2021, 3, 10).Date).ToListAsync<Order>();
            Assert.IsTrue(result.Any());
        }
        ...
        public async Task JoinQueryMultiOrderAndGroupTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            query.TakePage(1, 10).OrderByDescending((a, b) => a.CreateTime).OrderBy((a, b) => a.SerialNo);
            IEnumerable<Order> result = await query.Select((a, b) => a).ToListAsync<Order>();
            Assert.IsTrue(result.Any());
        }
        ...
        public void JoinExpressionOrderByDescendingTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IList<Order> models = query.Select((a, b) => a).Where((f, g) => f.Version > 10).OrderBy((f, g) => new { f.SerialNo, f.IsDelete, f.IsActive }).OrderByDescending((f, g) => f.CreateTime).ToList<Order>();
            Assert.IsNotNull(models);
            Assert.IsTrue(models.Any());
        }
        ...
        public void JoinGroupByTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IList<Order> models = query.Select((a, b) => new { a.SerialNo, a.IsDelete, a.CreateTime }).Where((f, g) => f.Version > 10).OrderBy((f, g) => new { f.SerialNo }).GroupBy((f, g) => f.SerialNo).GroupBy((f, g) => new { f.IsDelete }).GroupBy((f, g) => new Order { CreateTime = f.CreateTime }).ToList<Order>();
            Assert.IsNotNull(models);
            Assert.IsTrue(models.Any());
        }
        ...
        public void JoinHavingTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IList<Order> models = query.Select((a, b) => new { a.SerialNo, a.Version, a.IsDelete }).Where((f, g) => f.Version > 10).OrderBy((f, g) => new { f.SerialNo }).GroupBy((f, g) => f.SerialNo).GroupBy((f, g) => new { f.IsDelete, f.Version }).Having((f, g) => f.Version > 5).ToList<Order>();
            Assert.IsNotNull(models);
            Assert.IsTrue(models.Any());
        }
        ...
        public void JoinMaxNullableTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            int? max = query.Where((f, g) => f.DocId == Guid.Empty).Max((f, b) => (int?)f.Version);
        ...
        public void JoinMaxStringNullableTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            string max = query.Where((f, g) => f.DocId == Guid.Empty).Max((f, g) => f.SerialNo);
            Assert.IsNull(max);
        }
        ...
        public async Task JoinMaxNullableNoDataAsyncTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            int? max = await query.Where((f, g) => f.DocId == Guid.Empty).MaxAsync((f, b) => (int?)f.Version);
        ...
        public void JoinSumNullableTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            int? max = query.Where((f, g) => f.DocId == Guid.Empty).Sum((f, g) => (int?)f.Version);
            Assert.IsFalse(max > 0);
        }
        ...
        public void JoinDateAddHoursTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IList<Order> entities = query.Where((f, g) => f.CreateTime > DateTime.Now.AddHours(-500)).ToList<Order>();
            Assert.IsTrue(entities.Any());
        }
        ...
        public void JoinDateAddMinutesTest()
        {
            ...
            IList<Order> entities = query.Where((f, g) => f.CreateTime > date.AddMinutes(-500 * 60)).ToList<Order>();
            Assert.IsTrue(entities.Any());
        }
        ...
        public void JoinDateAddSecondsTest()
        {
            ...
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            DateTime date = DateTime.Now;
            IList<Order> entities = query.Where((f, g) => f.CreateTime > date.AddSeconds(-500 * 60 * 60)).ToList<Order>();
            Assert.IsTrue(entities.Any());
        }
        ...
        public void JoinDateAddMillisecondsTest()
        {
            ...
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            DateTime date = DateTime.Now;
            IList<Order> entities = query.Where((f, g) => f.CreateTime > date.AddMilliseconds(-500 * 60 * 60 * 1000)).ToList<Order>();
            Assert.IsTrue(entities.Any());
        }
        ...
        public void JoinBoolNotHasValueTest()
        {
            using IDbConnection connection = CreateConnection();
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IList<Order> entities = query.Where((f, g) => !f.DocId.HasValue).OrderBy((f, g) => f.CreateTime).Take(10).ToList<Order>();
            Assert.IsTrue(entities.Any());
            Assert.IsTrue(entities.All(f => !f.DocId.HasValue));
        }
        ...
        public void ExistsTest()
        {
            ...
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId);
            IList<Status> statusList = new List<Status> { Status.Running };
            IList<Order> orders = query.Where((f, d) => statusList.Contains(f.Status)).Exist<Buyer>((a, b, c) => a.BuyerId == c.Id && c.Code.Equals("50AD")).Select((f, g) => new Order { Id = f.Id, SerialNo = (f.Freight ?? 0).ToString(CultureInfo.InvariantCulture), Remark = f.SerialNo }).ToList<Order>();
        ...
        public void PageQueryTest()
        {
            using IDbConnection connection = CreateConnection();
            IList<Status> statusList = new List<Status> { Status.Running };
            JoinQuery<Order, Item> query = connection.JoinQuery<Order, Item>().On(JoinType.Left, (a, b) => a.Id == b.OrderId).Where((f, d) => statusList.Contains(f.Status)).Exist<Buyer>((a, b, c) => a.BuyerId == c.Id && c.Code.Equals("50AD")).OrderByDescending((a, b) => a.CreateTime).Select((f, g) => f).TakePage(1, 10);
