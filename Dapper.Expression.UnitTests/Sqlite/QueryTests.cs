...
namespace Dapper.Extensions.Expression.UnitTests.Sqlite
{
    ...
    public class QueryTests : SqliteBaseTest
    {
        ...
        public void DateAddDaysTest()
        {
            using IDbConnection connection = CreateConnection();
            Query<Order> query = connection.Query<Order>();
            int count = query.Where(f => f.CreateTime > DateTime.Now.AddDays(-20)).Count();
            Assert.IsTrue(count > 0);
        }

        [TestMethod]
        public void NullablePropertyDateAddDaysTest()
        {
            using IDbConnection connection = CreateConnection();
            Query<Order> query = connection.Query<Order>();
            int count = query.Where(f => f.CreateTime > f.UpdateTime.Value.AddDays(-20)).Count();
            Assert.IsTrue(count > 0);
        }

        [TestMethod]
        public void PropertyDateAddDaysTest()
        {
            using IDbConnection connection = CreateConnection();
            Query<Order> query = connection.Query<Order>();
            int count = query.Where(f => f.CreateTime > f.CreateTime.AddDays(-20)).Count();
            Assert.IsTrue(count > 0);
        }
        ...
        public void DateAddHoursTest()
        {
            using IDbConnection connection = CreateConnection();
            Query<Order> query = connection.Query<Order>();
            int count = query.Where(f => f.CreateTime > DateTime.Now.Date.AddHours(-500)).Count();
            Assert.IsTrue(count > 0);
        }

        [TestMethod]
        public void NullablePropertyDateAddHoursTest()
        {
            using IDbConnection connection = CreateConnection();
            Query<Order> query = connection.Query<Order>();
            int count = query.Where(f => f.CreateTime > f.UpdateTime.Value.AddHours(-500)).Count();
            Assert.IsTrue(count > 0);
        }

        [TestMethod]
        public void PropertyDateAddMinutesTest()
        {
            using IDbConnection connection = CreateConnection();
            Query<Order> query = connection.Query<Order>();
            int count = query.Where(f => f.CreateTime > f.CreateTime.AddMinutes(-500 * 60)).Count();
            Assert.IsTrue(count > 0);
        }
        ...
        public void DateAddSecondsTest()
        {
            ...
            int count = query.Where(f => f.CreateTime > date.Date.AddSeconds(-500 * 60 * 60)).Count();
            Assert.IsTrue(count > 0);
        }
